...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals), 2) AS average_winner_goals_round FROM games")"
...
echo "$($PSQL "SELECT DISTINCT year, name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE round='Final' ORDER BY year")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT DISTINCT name FROM teams WHERE name ILIKE 'Co%' ORDER BY name")"
